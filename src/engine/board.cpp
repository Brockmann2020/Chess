...
Board::Board() {
    ...
    // Set up pawns
    for (int i = 8; i < 16; ++i) {
        _squares[i] = Piece::White | Piece::Pawn;
        _squares[i + 40] = Piece::Black | Piece::Pawn;
    }
    ...
    // Set up other pieces
    for (int i = 0; i < 8; ++i) {
        _squares[i] = Piece::White | pieceOrder[i];
        _squares[i + 56] = Piece::Black | pieceOrder[i];
    }
}

const std::array<int, 64>& Board::getBoard() const {
    return _squares;
}
